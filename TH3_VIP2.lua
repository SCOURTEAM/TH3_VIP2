...
bot_idkeko = tonumber(bot_idkeko[1])
sudo_users = {sudo_add,bot_id}
URL33 = require('socket.url')
---------- {Show Files} -----------
red = '\27[31m' reset = '\27[m' Blue = "\27[34m" Green = "\27[32m"
local VI_P2 = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(VI_P2) do
...
local keko_add_sudo = redis:get('TH3_VIP2:'..bot_id..'sudoo'..msg.sender_user_id_..'')
...
local var = false 
local admin = database:sismember('TH3_VIP2:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local keko_add_sudo = redis:get('TH3_VIP2:'..bot_id..'sudoo'..user_id..'')
...
local var = false
local creator = database:sismember('TH3_VIP2:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('TH3_VIP2:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('TH3_VIP2:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('tshake:'..bot_id..'admins:', user_id)  
local owner = database:sismember('TH3_VIP2:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('TH3_VIP2:'..bot_id..'creator:'..chat_id, user_id)  
local vip = database:sismember('TH3_VIP2:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('TH3_VIP2:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('TH3_VIP2:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('TH3_VIP2:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('TH3_VIP2:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  {
  {text = ''..(channel_ts or "SCOUR TEAM")..'', url=''..(channel_user_ts or 't.me/SCOURTEAM')..''},
  },
  }
...
getUser(msg.sender_user_id_, VI_P2)
end
if value == "prore" then
function get_TH3_VIP2X(TH3_VIP2x1,keko333x2,TH3_VIP2x3)
local id_TH3_VIP2 = TH3_VIP2x2.sender_user_id_
function VI_P2(extra,result,success)
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'tshaketeam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(id_TH3_VIP2x, VI_P2)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_TH3_VIP2X)
end
...
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'SCOURTEAM')..')\n'
local VI_P2 = database:get("add"..bot_id)
if VI_P2 then
local VI_P2 = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(keko)
local data = json:decode(stats)
if (data.result and data.result.status == 'creator') then
if not database:get( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} معطله سابقا", 1, 'md')
end
openChat(msg.chat_id_,TH3_VIP2_info)
end
if database:get( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_)
function TH3_VIP2_info(V1,V2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تعطيل المجموعه {"..k2.title_.."}", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
database:srem("TH3_VIP2:gog"..bot_id, msg.chat_id_)
database:del( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_)
function TH3_VIP2_info2(V1,V2)
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
database:set('TH3_VIP2:'..bot_id.."group:link"..msg.chat_id_ ,link.result) 
link = link.result 
else 
link = "(لا يوجد)"
end
send(tostring((database:get("tshake"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتعطيل مجموعه \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..V2.title_..")\n📎┇رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
openChat(msg.chat_id_,TH3_VIP2_info2) 
--
end
else
...
local VI_P2 = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(keko)
local data2 = json:decode(stats)
local VI_P2 = database:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(VI_P2) or tonumber(data2.result) > tonumber(VI_P2))) then
if database:get( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_) then
function TH3_VIP2_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(V2.title_ or "").."} مفعله سابقا", 1, 'md')
end
openChat(msg.chat_id_,TH3_VIP2_info)
end
if not database:get( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_,true)
function TH3_VIP2_info(V1,V2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تفعيل المجموعه {"..(V2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,TH3_VIP2_info)
...
database:sadd('TH3_VIP2:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'TH3_VIP2:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('TH3_VIP2:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
openChat(msg.chat_id_,TH3_VIP2_info2) 
--
database:sadd("TH3_VIP2:gog"..bot_id, msg.chat_id_)
database:set( 'TH3_VIP2:'..bot_id.."enable:"..msg.chat_id_,true)
end end
getUser(msg.sender_user_id_, TSby)
end
...
if (text == "قفل التاك") then
  local tsX_o = database:get("lock_tag:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التاك")
  else
  tsX000("lock",msg,"☑┇تم قفل التاك")
  database:set("lock_tag:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الشارحه") then
  local tsX_o = database:get("lock_sarha:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الشارحه")
  else
  tsX000("lock",msg,"☑┇تم قفل الشارحه")
  database:set("lock_sarha:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التعديل") then
  local tsX_o = database:get("lock_edit:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التعديل")
  else
  tsX000("lock",msg,"☑┇تم قفل التعديل")
  database:set("lock_edit:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التكرار") then
  local tsX_o = database:get("lock_lllll:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التكرار")
  else
  tsX000("lock",msg,"☑┇تم قفل التكرار")
  database:set("lock_lllll:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المتحركه") then
  local tsX_o = database:get("lock_gif:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل المتحركه")
  else
  tsX000("lock",msg,"☑┇تم قفل المتحركه")
  database:set("lock_gif:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملفات") then
  local tsX_o = database:get("lock_files:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الملفات")
  else
  tsX000("lock",msg,"☑┇تم قفل الملفات")
  database:set("lock_files:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الماركدون") then
  local tsX_o = database:get("lock_mark:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الماركدون")
  else
  tsX000("lock",msg,"☑┇تم قفل الماركدون")
  database:set("lock_mark:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصور") then
  local tsX_o = database:get("lock_photo:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الصور")
  else
  tsX000("lock",msg,"☑┇تم قفل الصور")
  database:set("lock_photo:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملصقات") then
  local tsX_o = database:get("lock_stecker:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الملصقات")
  else
  tsX000("lock",msg,"☑┇تم قفل الملصقات")
  database:set("lock_stecker:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفيديو") then
  local tsX_o = database:get("lock_video:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الفيديو")
  else
  tsX000("lock",msg,"☑┇تم قفل الفيديو")
  database:set("lock_video:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانلاين") then
  local tsX_o = database:get("lock_inline:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الانلاين")
  else
  tsX000("lock",msg,"☑┇تم قفل الانلاين")
  database:set("lock_inline:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الدردشه") then
  local tsX_o = database:get("lock_chat:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الدردشه")
  else
  tsX000("lock",msg,"☑┇تم قفل الدردشه")
  database:set("lock_chat:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التوجيه") then
  local tsX_o = database:get("lock_fwd:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التوجيه")
  else
  tsX000("lock",msg,"☑┇تم قفل التوجيه")
  database:set("lock_fwd:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التثبيت") then
  local tsX_o = database:get("lock_pin:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التثبيت")
  else
  tsX000("lock",msg,"☑┇تم قفل التثبيت")
  database:set("lock_pin:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاغاني") then
  local tsX_o = database:get("lock_audeo:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الاغاني")
  else
  tsX000("lock",msg,"☑┇تم قفل الاغاني")
  database:set("lock_audeo:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصوت") then
  local tsX_o = database:get("lock_voice:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الصوت")
  else
  tsX000("lock",msg,"☑┇تم قفل الصوت")
  database:set("lock_voice:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الجهات") then
  local tsX_o = database:get("lock_contact:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الجهات")
  else
  tsX000("lock",msg,"☑┇تم قفل الجهات")
  database:set("lock_contact:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل العربيه") then
  local tsX_o = database:get("lock_ar:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┇بالفعل تم قفل العربيه")
  else
  tsX000("lock",msg,"☑┇تم قفل العربيه")
  database:set("lock_ar:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانكليزيه") then
  local tsX_o = database:get("lock_en:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الانكليزيه")
  else
  tsX000("lock",msg,"☑┇تم قفل الانكليزيه")
  database:set("lock_en:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الميديا") then
  local tsX_o = database:get("lock_media:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الميديا")
  else
  tsX000("lock",msg,"☑┇تم قفل الميديا")
  database:set("lock_media:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الروابط") then
  local tsX_o = database:get("lock_link:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┇بالفعل تم قفل الروابط")
  else
  tsX000("lock",msg,"☑┇تم قفل الروابط")
  database:set("lock_link:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المعرف") then
  local tsX_o = database:get("lock_username:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┇بالفعل تم قفل المعرف")
  else
  tsX000("lock",msg,"☑┇تم قفل المعرف")
  database:set("lock_username:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاشعارات") then
  local tsX_o = database:get("lock_new:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الاشعارات")
  else
  tsX000("lock",msg,"☑┇تم قفل الاشعارات")
  database:set("lock_new:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات بالطرد") then
  local tsX_o = database:get("lock_botAndBan:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات بالطرد")
  else
  tsX000("lock",msg,"☑┇تم قفل البوتات بالطرد")
  database:set("lock_botAndBan:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات") then
  local tsX_o = database:get("lock_bot:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات")
  else
  tsX000("lock",msg,"☑┇تم قفل البوتات")
  database:set("lock_bot:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل بصمه الفيديو") then
  local tsX_o = database:get("lock_note:TH3_VIP2"..msg.chat_id_..bot_id)
  if tsX_o then
  ...
  database:del("lock_new:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_tag:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_sarha:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_lllll:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_edit:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_gif:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_files:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_mark:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_photo:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_stecker:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_video:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_inline:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_chat:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_fwd:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_pin:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_audeo:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_voice:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_botAndBan:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_contact:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_ar:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_en:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_media:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_username:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_link:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_bot:TH3_VIP2"..msg.chat_id_..bot_id,"ok")
  end
  end  --         »»               End TH3_VIP2 unlock                       ««              --
  end 
end 
...
if database:get("lock_link:tTH3_VIP2"..msg.chat_id_..bot_id) then
...
if text and (text:match("[Hh][Tt][Tt][Pp][Ss]://") or text:match("[Hh][Tt][Tt][Pp]://") or text:match(".[Ii][Rr]") or text:match(".[Cc][Oo][Mm]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Nn][Ff][Oo]") or text:match("[Ww][Ww][Ww].") or text:match(".[Tt][Kk]")) then
if database:get("lock_link:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_username:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_chat:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "TH3_VIP2"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:TH3_VIP2"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:TH3_VIP2"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("lock_fwd:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
if msg.content_.ID == "MessageSticker" then
if database:get("lock_stecker:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end

if msg.content_.ID == "MessageChatAddMembers" then
database:incr('TH3_VIP2:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
...
if database:get("lock_mark:TH3_VIP2"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_lllll:TH3_VIP2"..msg.chat_id_..bot_id) then
...
local hash = 'TH3_VIP2:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
if not database:get('TH3_VIP2:'..bot_id..'leave:groups') then
...
local hash =  'TH3_VIP2:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('TH3_VIP2:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'SCOURTEAM')..')\n☑┇تم رفع منشئ في البوت'
else
texts = '✖┇خطاء'
end
...
database:sadd('TH3_VIP2:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
local user_info = database:hgetall('TH3_VIP2:'..bot_id..'user:'..v)
...
if not database:get('TH3_VIP2:'..bot_id..'bc:groups') then
local gps = database:scard( 'TH3_VIP2:'..bot_id.."groups") or 0
local gpss = database:smembers( 'TH3_VIP2:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('TH3_VIP2:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, ssss[2], 1, 'html')		
end					
end
local pro = database:scard('TH3_VIP2:'..bot_id..'pro:groups') or 0
...
local gps = database:scard('TH3_VIP2:'..bot_id.."groups") or 0
local user = database:scard('TH3_VIP2:'..bot_id.."userss") or 0
local gps2 = database:scard("TH3_VIP2:gog"..bot_id) or 0
local gps3 = database:scard('TH3_VIP2:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('TH3_VIP2:'..bot_id.."groups") or 0
local gps2 = database:smembers("TH3_VIP2:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("TH3_VIP2:gog"..bot_id, gps[i])
database:set('TH3_VIP2:'..bot_id.."enable:"..gps[i],true)
database:set( 'TH3_VIP2:'..bot_id.."charge:"..gps[i],true)
end
...
local gps2 = database:smembers("TH3_VIP2:gog"..bot_id) or 0
for i=1,#gps do
database:del("TH3_VIP2:gog"..bot_id)
database:del('TH3_VIP2:'..bot_id.."enable:"..gps[i])
database:del( 'TH3_VIP2:'..bot_id.."charge:"..gps[i])
end
...
database:del("TH3_VIP2:gog"..bot_id)
database:del('TH3_VIP2:'..bot_id.."groups")
end
end --
if text:match("^روابط الكروبات$") then
local gpss = database:smembers("TH3_VIP2:"..bot_id.."groups") or 0
local gps = database:scard('TH3_VIP2:'..bot_id.."groups")
text = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('TH3_VIP2:'..bot_id.."group:link"..gpss[i])
text = text.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
 local f = io.open('TH3_VIP2.txt', 'w')
 f:write(text)
 f:close()
 local tshakee = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
 local curl = 'curl "' .. TH3_VIP2 .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'TH3_VIP2.txt' .. '"'
 io.popen(curl)
 end
if text:match("^تحديث السورس$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم التحديث', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('cd libs && wget https://raw.githubusercontent.com/SCOURTEAM/TH3_VIP2/master/libs/utils.lua')
os.execute('rm -rf TH3_VIP2.lua')
os.execute('wget https://raw.githubusercontent.com/SCOURTEAM/TH3_VIP2/master/TH3_VIP2.lua')
os.exit()
return false
end
...
local time = a[2] * day
database:setex( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'TH3_VIP2:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'TH3_VIP2:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:del("TH3_VIP2:gog"..bot_id,txt[2])
...
local keko_info = nil
function VI_P2(extra,result,success)
VI_P2_info = '@'..(result.username_ or 'لا يوجد')..''
local timeplan1 = 2592000
database:setex( 'TH3_VIP2:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
for k,v in pairs(sudo_users) do
function TH3_VIP2_info(k1,k2)
send(v, 0, 1, "🔘┇قام بتفعيل مجموعه المده كانت 30 يوم \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ "..keko_info.."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ┇اسم المجموعه ~⪼ ("..V2.title_..")" , 1, 'md')
end
openChat(msg.chat_id_,TH3_VIP2_info)
end
database:set( 'TH3_VIP2:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, VI_P2)
end
...
database:setex( 'TH3_VIP2:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
database:set( 'TH3_VIP2:'..bot_id.."charge:"..txt[2],true)
...
local files_TH3_VIP2 = database:smembers("files"..bot_id)
local keko = io.popen('cd files_TH3_VIP2 && ls'):read("*all")
local files_TH3_VIP22 = ''
for i=1,#files_TH3_VIP22 do
files_TH3_VIP22 = files_TH3_VIP22..'{'..files_TH3_VIP2[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..VI_P2..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_TH3_VIP22..'', 1, 'html')
end
text = msg.content_.text_
if text then
...
local name_t = {string.match(text, "^(حذف ملف) (.*)(.lua)$")}
io.popen("rm -fr TH3_VIP2/"..name_t[2]..'.lua')
database:srem("files"..bot_id,name_t[2]..'.lua')
...
local tshakee = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. TH3_VIP2 .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'TH3_VIP2/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
end -- end if sudo
...
function TH3_VIP2_run_file(data)
local TH3_VIP2 = database:smembers("files"..bot_id)
for i=1,#TH3_VIP2 do
local TH3_VIP2 = dofile("files_tshake/"..TH3_VIP2[i])
local kt = TH3_VIP2.VI_P2_TH3_VIP2(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('TH3_VIP2:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('TH3_VIP2:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
return false end
  ...
  if not database:get( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_) then
  if database:get( 'TH3_VIP2:'..bot_id.."enable:"..msg.chat_id_) then
  database:del( 'TH3_VIP2:'..bot_id.."enable:"..msg.chat_id_)
  for k,v in pairs(sudo_users) do
  end
  end
  end
    ...
    if not database:sismember( 'TH3_VIP2:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'TH3_VIP2:'..bot_id.."groups",msg.chat_id_)
    end
    elseif id:match('^(%d+)') then
    database:sadd('TH3_VIP2:'..bot_id.."userss",msg.chat_id_)
    else
    if not database:sismember( 'TH3_VIP2:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'TH3_VIP2:'..bot_id.."groups",msg.chat_id_)
    end
  ...
  database:incr('TH3_VIP2:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
  if database:get('TH3_VIP2:'..bot_id..'viewget'..msg.sender_user_id_) then
  if not msg.forward_info_ then
  send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
  database:del('TH3_VIP2:'..bot_id..'viewget'..msg.sender_user_id_)
  else
  ...
  local hash =  'TH3_VIP2:'..bot_id..'mods:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
  ...
  if result.id_ then
  database:sadd('TH3_VIP2:'..bot_id..'mods:'..msg.chat_id_, result.id_)
  ...
  database:sadd('TH3_VIP2:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
  ...
  if result.id_ then
  database:srem(hash, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SCOURTEAM')..')\n☑┇تم تنزيله من ادمنيه البوت'
  else
  texts = '✖┇خطاء'
  end
  send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
  ...
  function promote_by_reply(extra, result, success)
  local hash =  'TH3_VIP2:'..bot_id..'vipgp:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┇بالفعل تم رفعه عضو مميز") 
  ...
  function promote_by_username(extra, result, success)
  if result.id_ then
  database:sadd('TH3_VIP2:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SCOURTEAM')..')\n☑┇تم رفعه عضو مميز'
  else
  texts = '✖┇خطاء'
  end
  ...
  database:sadd('TH3_VIP2:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
  ...
  database:set("TH3_VIP2:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'tshake')
  return "TH3_VIP2"
  end
  if text and is_owner(msg) and database:get("TH3_VIP2:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
  database:del("TH3_VIP2:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_)
  send(msg.chat_id_, msg.id_, 1,  '☑┇ تم الحفض بنجاح', 1, 'md')
  database:set("TH3_VIP2:gr:id:text:"..bot_id..msg.chat_id_,text)
  end
  if text and text == "مسح الايدي" and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
  database:del("TH3_VIP2:gr:id:text:"..bot_id..msg.chat_id_)
  end
  ...
  tsX000(apmd[2],msg,"☑┇تم تنزيله من اعضاء الممزين البوت")
    ...
    local hash =  'TH3_VIP2:'..bot_id..'banned:'..msg.chat_id_
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('TH3_VIP2:'..bot_id..'banned:'..msg.chat_id_, result.id_)
      texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'SCOURTEAM')..')\n☑┇تم حظره من المجموعه'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('TH3_VIP2:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  if database:get('TH3_VIP2:'..bot_id.."group:link"..msg.chat_id_) == 'Waiting For Link!\nPls Send Group Link' and is_mod(msg) then 
  ...
  local hash = 'TH3_VIP2:'..bot_id.."group:link"..msg.chat_id_ 
  ...
  send(msg.chat_id_, 0, 1, '↙️┇رابط المجموعه الجديد\n'..glink, 1, 'html')
   ...
   if result.id_ then
     database:srem('TH3_VIP2:'..bot_id..'banned:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'SCOURTEAM')..')\n☑┇تم الغاء حظره من البوت' 
   ...
   local apba = {string.match(text, "^(الغاء حظر) (%d+)$")}
     database:srem('TH3_VIP2:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  function gban_by_reply(extra, result, success)
    local hash =  'TH3_VIP2:'..bot_id..'gbanned:'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر عام \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'TH3_VIP2:'..bot_id..'gbanned:'
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'SCOURTEAM')..')\n🚫┇تم حظره من المجموعات البوت'
  ...
  function ungban_by_username(extra, result, success)
    local hash =  'TH3_VIP2:'..bot_id..'gbanned:'
    if result.id_ then
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'SCOURTEAM')..')\n🚫┇تم الغاء حظره من المجموعات البوت'
  database:srem(hash, result.id_)
    else
    texts = '✖┇خطاء'
    end
  ...
  function gmute_by_reply(extra, result, success)
    local hash =  'TH3_VIP2:'..bot_id..'gmuted:'
    if is_admin(result) then
...
tsX000("prore",msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
  ...
  tsX000(apbll[2],msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
    ...
    local hash =  'TH3_VIP2:'..bot_id..'muted:'..msg.chat_id_
    if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
tsX000("prore",msg,"🚫┇بالفعل تم كتمه")
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع كتم \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('TH3_VIP2:'..bot_id..'muted:'..msg.chat_id_, result.id_)
    texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'SCOURTEAM')..')\n🚫┇تم كتمه من البوت'
    end
  else
    texts = '✖┇خطاء'
  end
...
database:sadd('TH3_VIP2:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
tsX000(apsi[2],msg,"🚫┇تم كتمه من البوت")
    ...
    if result.id_ then
     database:srem('TH3_VIP2:'..bot_id..'muted:'..msg.chat_id_, result.id_)
   texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'SCOURTEAM')..')\n🚫┇تم الغاء كتمه من البوت'
    ...
    local apsi = {string.match(text, "^(الغاء كتم) (%d+)$")}
      database:srem('TH3_VIP2:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'SCOURTEAM')..')\n🚫┇تم طرده من المجموعه'
       chat_kick(msg.chat_id_, result.id_)
    end
  else
    texts = '✖┇خطاء'
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'TH3_VIP2:'..bot_id..'owners:'..msg.chat_id_
    if database:sismember(hash, result.sender_user_id_) then
    ...
    if result.id_ then
      database:sadd('TH3_VIP2:'..bot_id..'owners:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'SCOURTEAM')..')\n☑┇تم رفع مدير في البوت'
  ...
  if text:match("^قائمه المنع$") and is_mod(msg) then
    local hash =  'TH3_VIP2:'..bot_id..'filters:'..msg.chat_id_
  if hash then
     local names = database:hkeys(hash)
  ...
  if text:match("^جلب صوره (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('TH3_VIP2:'..bot_id..'get:photo'..msg.chat_id_) then
...
local ph = pronumb[2] - 1
  ...
  database:set('TH3_VIP2:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
  ...
  database:set('TH3_VIP2:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
     send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع الزمن التكرار للعدد ~⪼  *{'..floodt[2]..'}*', 1, 'md')
    end
    end
    
    if text:match("^وضع رابط$") and is_mod(msg) then
     database:set( 'TH3_VIP2:'..bot_id.."group:link"..msg.chat_id_, 'Waiting For Link!\nPls Send Group Link')
     send(msg.chat_id_, msg.id_, 1, '📮┇قم بارسال الرابط  ليتم حفظه\n', 1, 'md')
    end
    
    if text:match("^الرابط$") then
    local link = database:get( 'TH3_VIP2:'..bot_id.."group:link"..msg.chat_id_)
      if link then
    ...
    local welcome = {string.match(text, "^(وضع ترحيب) (.*)$")}
     send(msg.chat_id_, msg.id_, 1, '☑┇تم وضع ترحيب\n📜┇~⪼('..welcome[2]..')', 1, 'md')
       database:set('TH3_VIP2:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
    end
    ...
    local wel = database:get('TH3_VIP2:'..bot_id..'welcome:'..msg.chat_id_)
    if wel then
  ...
  database:hset('TH3_VIP2:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
  ...
  database:hdel('TH3_VIP2:'..bot_id..'filters:'..msg.chat_id_, rws[2])
  ...
  if (text:match("^كشف البوتات$") or text:match("^البوتات$")) and is_mod(msg) then
   ...
   n = (n + 1)
     local user_info = database:hgetall('TH3_VIP2:'..bot_id..'user:'..v.user_id_)
  if user_info and user_info.username then
   local username = user_info.username
...
text = ""
  ...
  local user_msgs = database:get('TH3_VIP2:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
   if not database:get('TH3_VIP2:'..bot_id..'id:mute'..msg.chat_id_) then
...
add = (tonumber(database:get('TH3_VIP2:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
  send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('TH3_VIP2:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
  end
  if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
  local edit = database:get('TH3_VIP2:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
  ...
  database:del('TH3_VIP2:'..bot_id..'banned:'..msg.chat_id_)
  ...
  database:del('TH3_VIP2:'..bot_id..'creator:'..msg.chat_id_)
...
text = ""
  ...
  send(msg.chat_id_, msg.id_, 1, moody, 1, 'html')
   ...
   bot.channel_get_admins(msg.chat_id_,cb)
    ...
    if database:get("lock_media:TH3_VIP2"..msg.chat_id_..bot_id) then
       mute_all = '✔┇'
       else
       mute_all = '✖┇'
       end
       ...
       ------------
       if not database:get('TH3_VIP2:'..bot_id..'flood:max:'..msg.chat_id_) then
       flood_m = 10
       else
       flood_m = database:get('TH3_VIP2:'..bot_id..'flood:max:'..msg.chat_id_)
     end
       if not database:get( 'TH3_VIP2:'..bot_id..'flood:time:'..msg.chat_id_) then
       flood_t = 10
       else
       flood_t = database:get( 'TH3_VIP2:'..bot_id..'flood:time:'..msg.chat_id_)
     end
       ------------
       if database:get("lock_audeo:TH3_VIP2"..msg.chat_id_..bot_id) then
       mute_music = '✔┇'
       ...
       ------------
       if database:get("lock_inline:TH3_VIP2"..msg.chat_id_..bot_id) then
       mute_in = '✔┇'
       else
       mute_in = '✖┇'
       end
       ...
       ------------
       if database:get("lock_edit:TH3_VIP2"..msg.chat_id_..bot_id) then
       mute_edit = '✔┇'
       else
       mute_edit = '✖┇'
       end
   ...
   ------------
       if database:get("lock_pin:TH3_VIP2"..msg.chat_id_..bot_id) then
       lock_pin = '✔┇'
       else
       lock_pin = '✖┇'
     end
  ...
  if database:get("lock_contact:TH3_VIP2"..msg.chat_id_..bot_id) then
   ...
   if database:get('TH3_VIP2:'..bot_id..'rep:mute'..msg.chat_id_) then
       lock_rep = '✔┇'
       else
       lock_rep = '✖┇'
       end
       ------------
   if database:get('TH3_VIP2:'..bot_id..'repsudo:mute'..msg.chat_id_) then
       lock_repsudo = '✔┇'
       else
       lock_repsudo = '✖┇'
       end
       ------------
   if database:get('TH3_VIP2:'..bot_id..'repowner:mute'..msg.chat_id_) then
       lock_repowner = '✔┇'
       else
       lock_repowner = '✖┇'
       end
       ------------
   if database:get('TH3_VIP2:'..bot_id..'id:mute'..msg.chat_id_) then
       lock_id = '✔┇'
       else
       lock_id = '✖┇'
       end
       ------------
   if database:get('TH3_VIP2:'..bot_id..'pin:mute'..msg.chat_id_) then
       lock_pind = '✔┇'
       else
       lock_pind = '✖┇'
       end
   ...
   if database:get('TH3_VIP2:'..bot_id..'id:mute'..msg.chat_id_) then
       ...
       ------------
if database:get( 'TH3_VIP2:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
    ------------
local ex = database:ttl( 'TH3_VIP2:'..bot_id.."charge:"..msg.chat_id_)
  if ex == -1 then
  exp_dat = 'لا نهائي'
  else
  ...
  if (text and text == 'تفعيل اطردني') and is_owner(msg) then
     if not database:get('TH3_VIP2:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تفعيله', 1, 'md')
     else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل امر اطردني', 1, 'md')
    database:del('TH3_VIP2:'..bot_id..'kickme:mute'..msg.chat_id_)
   end
   end
   if (text and text == 'تعطيل اطردني') and is_owner(msg) then
     if database:get('TH3_VIP2:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تعطيله', 1, 'md')
   else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل امر اطردني', 1, 'md')
     database:set('TH3_VIP2:'..bot_id..'kickme:mute'..msg.chat_id_,true)
   end
     end

  if text:match("^اطردني$") then
  if not database:get('TH3_VIP2:'..bot_id..'kickme:mute'..msg.chat_id_) then
  redis:set('TH3_VIP2:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:set('TH3_VIP2:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '🚷┇ ارسل ؛ نعم ، ليتم طردك\n🔘┇ارسل ؛ لا ، لالغاء الامر', 1, 'md')
  else
  send(msg.chat_id_, msg.id_, 1, '⚠┇تم تعطيل امر اطردني', 1, 'md')
  end
  end
  local yess = redis:get('TH3_VIP2:'..bot_id..'kickyess'..msg.sender_user_id_..'')
  ...
  if yess == 'kickyes' then
  chat_kick(msg.chat_id_, msg.sender_user_id_)
  redis:del('TH3_VIP2:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('TH3_VIP2:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '✅┇تم طردك من المجموعه', 1, 'md')
    end
    end
  end
  if text:match("^لا$") then
  local noo = redis:get('TH3_VIP2:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
  if noo == 'kickno' then
  redis:del('TH3_VIP2:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('TH3_VIP2:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  ...
  redis:set('TH3_VIP2:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local keko2 = redis:get('TH3_VIP2:'..bot_id..'texts'..msg.sender_user_id_..'')
  if keko2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('TH3_VIP2:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
  redis:set('TH3_VIP2:'..bot_id..'text_sudo', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
  if  (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   redis:del('TH3_VIP2:'..bot_id..'text_sudo', text)
  ...
  if text:match("^[Dd][Ee][Vv]$")or text:match("^مطور بوت$") or text:match("^مطورين$") or text:match("^مطور البوت$") or text:match("^مطور$") or text:match("^المطور$") and msg.reply_to_message_id_ == 0 then
    local text_sudo = redis:get('TH3_VIP2:'..bot_id..'text_sudo')
  local nkeko = redis:get('TH3_VIP2:'..bot_id..'nmkeko')
  local nakeko = redis:get('TH3_VIP2:'..bot_id..'nakeko')
    if text_sudo then
    send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
    else
    sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkeko or 9640780387389), (nakeko or "SCOURTEAM"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
  ...
  redis:set('TH3_VIP2:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'msg')
    return false end
  end
  if text:match("^+(.*)$") then
  local kekoo = redis:get('TH3_VIP2:'..bot_id..'sudoo'..text..'')
  local keko2 = redis:get('TH3_VIP2:'..bot_id..'nkeko'..msg.sender_user_id_..'')
  if keko2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
  redis:set('TH3_VIP2:'..bot_id..'nmkeko', text)
  redis:set('TH3_VIP2:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'mmsg')
    return false end
  end
  if text:match("^(.*)$") then
  local keko2 = redis:get('TH3_VIP2:'..bot_id..'nkeko'..msg.sender_user_id_..'')
  ...
  redis:set('TH3_VIP2:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'no')
  redis:set('TH3_VIP2:'..bot_id..'nakeko', text)
  local nmkeko = redis:get('TH3_VIP2:'..bot_id..'nmkeko')
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmkeko, text , "", bot_id)
    ...
    function promote_by_reply(extra, result, success)
    if redis:sismember('TH3_VIP2:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┇بالفعل تم رفعه مطور')
  else
    redis:set('TH3_VIP2:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
    redis:sadd('TH3_VIP2:'..bot_id..'dev', result.sender_user_id_)
    ...
    function promote_by_username(extra, result, success)
    if result.id_ then
    redis:set('TH3_VIP2:'..bot_id..'sudoo'..result.id_..'', 'yes')
    redis:sadd('TH3_VIP2:'..bot_id..'dev', result.id_)
      texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SCOURTEAM')..')\n\n☑┇تم رفعه مطور'
  else
    texts = '✖┇خطاء'
  end
    ...
    redis:set('TH3_VIP2:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
    redis:sadd('TH3_VIP2:'..bot_id..'dev', apmd[2])
    ...
    function demote_by_reply(extra, result, success)
    if not redis:sismember('TH3_VIP2:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┇ بالفعل تم تنزيله من المطورين')
    else
    redis:del('TH3_VIP2:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
    redis:srem('TH3_VIP2:'..bot_id..'dev', result.sender_user_id_)
    ...
    function demote_by_username(extra, result, success)
    if result.id_ then
    redis:del('TH3_VIP2:'..bot_id..'sudoo'..result.id_..'', 'no')
    redis:srem('TH3_VIP2:'..bot_id..'dev', result.id_)
    ...
    redis:del('TH3_VIP2:'..bot_id..'sudoo'..apmd[2]..'', 'no')
    redis:srem('TH3_VIP2:'..bot_id..'dev', apmd[2])
  ...
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('TH3_VIP2:'..bot_id..'repowner:mute'..msg.chat_id_) then
  local keko = redis:get('TH3_VIP2:'..bot_id..'VI_P2'..text..''..msg.chat_id_..'')
  send(msg.chat_id_, msg.id_, 1, keko, 1, 'md')
  end
  local keko1 = redis:get('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..''..msg.chat_id_..'')
  if keko1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
  redis:set('TH3_VIP2:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
  redis:sadd('TH3_VIP2:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
    return false end
  if keko1 == 're' then
  local keko2 = redis:get('TH3_VIP2:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..keko2..''..msg.chat_id_..'', text)
  redis:sadd('TH3_VIP2:'..bot_id..'VI_P2'..msg.chat_id_..'', keko2)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  end
  end
    if text:match("^حذف رد$") and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها', 1, 'md')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local keko1 = redis:get('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..''..msg.chat_id_..'')
  if keko1 == 'nomsg' then
  ...
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..text..''..msg.chat_id_..'', " ")
 redis:srem('TH3_VIP2:'..bot_id..'VI_P2'..msg.chat_id_..'', text)
   end
  end
  ...
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('TH3_VIP2:'..bot_id..'repsudo:mute'..msg.chat_id_) then
  local keko = redis:get('TH3_VIP2:'..bot_id..'VI_P2'..text..'')
  send(msg.chat_id_, msg.id_, 1, keko, 1, 'md')
  end
  local keko1 = redis:get('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..'')
  if keko1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..'', 're')
  redis:set('TH3_VIP2:'..bot_id..'msg'..msg.sender_user_id_..'', text)
    return false end
  if VI_P2 == 're' then
  local VI_P2 = redis:get('TH3_VIP2:'..bot_id..'msg'..msg.sender_user_id_..'')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..VI_P2..'', text)
  redis:sadd('TH3_VIP2:'..bot_id..'VI_P2resudo', keko2)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..'', 'no')
  end
  end

  if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها' , 1, 'md')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local keko1 = redis:get('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..'')
  if keko1 == 'nomsg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..msg.sender_user_id_..'', 'no')
   redis:set('TH3_VIP2:'..bot_id..'VI_P2'..text..'', " ")
   redis:srem('TH3_VIP2:'..bot_id..'VI_P2resudo', text)
   end
  end

  if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    local list = redis:smembers('TH3_VIP2:'..bot_id..'dev')
    for k,v in pairs(list) do
  redis:del('TH3_VIP2:'..bot_id..'dev', text)
  redis:del('TH3_VIP2:'..bot_id..'sudoo'..v..'', 'no')
  end
    send(msg.chat_id_, msg.id_, 1, "☑┇تم مسح مطورين البوت", 1, 'md')
    end
    if text:match("^مسح ردود المدير$") and is_owner(msg) then
    local list = redis:smembers('TH3_VIP2:'..bot_id..'VI_P2'..msg.chat_id_..'')
    for k,v in pairs(list) do
  redis:del('TH3_VIP2:'..bot_id..'VI_P2'..msg.chat_id_..'', text)
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..v..''..msg.chat_id_..'', " ")
  end
    ...
    local list = redis:smembers('VI_P2:'..bot_id..'VI_P2resudo')
    for k,v in pairs(list) do
  redis:del('TH3_VIP2:'..bot_id..'VI_P2resudo', text)
  redis:set('TH3_VIP2:'..bot_id..'VI_P2'..v..'', " ")
  end
  ...
  if text:match("^ردود المطور$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
    local list = redis:smembers('TH3_VIP2:'..bot_id..'VI_P2resudo')
  ...
  text = "❕┇لا يوجد ردود للمطور"
    ...
    local list = redis:smembers('TH3_VIP2:'..bot_id..'VI_P2re'..msg.chat_id_..'')
  ...
  text = "❕┇لا يوجد ردود للمدير"
    ...
    database:set('TH3_VIP2:'..bot_id..'rules'..msg.chat_id_, txt[2])
     send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
    end
    
      if  text:match("^القوانين$") then
    local rules = database:get('TH3_VIP2:'..bot_id..'rules'..msg.chat_id_)
    if rules then
  ...
  database:set('TH3_VIP2:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if (text and text == 'تفعيل جلب صوره') and is_owner(msg) then
  ...
  if database:sismember('TH3_VIP2:'..bot_id..'pro:groups',msg.chat_id_) then
    ...
    send(msg.chat_id_, msg.id_, 1, '☑┇تم اضافه المجموعه الى الوضع المدفوع', 1, 'md')
     database:sadd('TH3_VIP2:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  if   (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
  if not database:sismember('TH3_VIP2:'..bot_id..'pro:groups',msg.chat_id_) then
  ...
  database:srem('TH3_VIP2:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المطور', 1, 'md')
   database:del('TH3_VIP2:'..bot_id..'repsudo:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, 'تم تعطيل ردود المطور', 1, 'md')
    database:set('TH3_VIP2:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المدير', 1, 'md')
   database:del('TH3_VIP2:'..bot_id..'repowner:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل ردود المدير', 1, 'md')
    database:set('TH3_VIP2:'..bot_id..'repowner:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل الايدي', 1, 'md')
   database:del('TH3_VIP2:'..bot_id..'id:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل الايدي', 1, 'md')
    database:set('TH3_VIP2:'..bot_id..'id:mute'..msg.chat_id_,true)
  end
  end
  ...
  database:set("TH3_VIP2"..bot_id..":sudo:gr",msg.chat_id_)
  end
  ...
  database:del("TH3_VIP2"..bot_id..":sudo:gr")
  end
  if text:match("^ايدي$") and msg.reply_to_message_id_ == 0 then
  local keko_info = nil
  function keko333(extra,result,success)
...
VI_P2 = math.random(#VI_P2_text)
  sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_," 📷┇ "..VI_P2_text[VI_P2].."\n🎫┇ايديك ~⪼ ("..msg.sender_user_id_..")\n📜┇معرفك ~⪼ "..VI_P2_info.."\n📡┇موقعك ~⪼ "..t.."\n📨┇رسائلك ~⪼ {"..user_msgs.."}\n📧┇السحكات ~⪼ {"..edit.."}\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ",msg.id_,msg.id_.."")
  else 
  ...
  local TH3_VIP2_new_text = database:get("tshake:gr:id:text:"..bot_id..msg.chat_id_)
  local TH3_VIP2_new_text = tshake_new_text:gsub('#username',(keko_info or 'لا يوجد'))
  local TH3_VIP2_new_text = TH3_VIP2_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
  local TH3_VIP2_new_text = TH3_VIP2_new_text:gsub('#edit',(edit or 'لا يوجد'))
  local TH3_VIP2_new_text = TH3_VIP2_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
  local TH3_VIP2_new_text = tshake_new_text:gsub('#stast',(t or 'لا يوجد'))   
  send(msg.chat_id_, msg.id_, 1, TH3_VIP2_new_text, 1, 'html')
  end   
  end
  else
  end
  ...
  if not database:get('TH3_VIP2:'..bot_id..'id:mute'..msg.chat_id_) then
  if not database:get('TH3_VIP2:'..bot_id..'id:photo'..msg.chat_id_) then
  if not database:get("TH3_VIP2:gr:id:text:"..bot_id..msg.chat_id_) then 
  send(msg.chat_id_, msg.id_, 1, "❕┇انت لا تملك صوره لحسابك\n🎫┇ايديك ~⪼ ("..msg.sender_user_id_..")\n📜┇معرفك ~⪼ "..VI_P2_info.."\n📡┇موقعك ~⪼ "..t.."\n📨┇رسائلك ~⪼ <b>{"..user_msgs.."}</b>\n📧┇السحكات ~⪼ <b>{"..edit.."}</b>\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ", 1, 'html')
  else 
  local TH3_VIP2_new_text = database:get("TH3_VIP2:gr:id:text:"..bot_id..msg.chat_id_)
  local TH3_VIP2_new_text = TH3_VIP2_new_text:gsub('#username',(VI_P2_info or 'لا يوجد'))
  local TH3_VIP2_new_text = TH3_VIP2_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
  local TH3_VIP2_new_text = TH3_VIP2_new_text:gsub('#edit',(edit or 'لا يوجد'))
  local TH3_VIP2_new_text = TH3_VIP2_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
  local TH3_VIP2_new_text = TH3_VIP2_new_text:gsub('#stast',(t or 'لا يوجد'))   
  send(msg.chat_id_, msg.id_, 1, TH3_VIP2_new_text, 1, 'html')
  end
  else
  if is_sudo(msg) then
  ...
  if  (text and text == 'تفعيل التثبيت') and is_owner(msg) then
     if not database:get('TH3_VIP2:'..bot_id..'pin:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇التثبيت بالفعل تم تفعيله', 1, 'md')
     else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل التثبيت', 1, 'md')
    database:del('TH3_VIP2:'..bot_id..'pin:mute'..msg.chat_id_)
   end
   end
   ...
   pin(msg.chat_id_,msg.reply_to_message_id_,0)
       database:set('TH3_VIP2:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
    ...
    database:set('TH3_VIP2:'..bot_id..'viewget'..msg.sender_user_id_,true)
  ...
  redis:del('TH3_VIP2:'..bot_id..'help', text)
  redis:del('TH3_VIP2:'..bot_id..'h1', text)
  redis:del('TH3_VIP2:'..bot_id..'h2', text)
  redis:del('TH3_VIP2:'..bot_id..'h3', text)
  redis:del('TH3_VIP2:'..bot_id..'h4', text)
  redis:del('TH3_VIP2:'..bot_id..'h5', text)
  redis:del('TH3_VIP2:'..bot_id..'h6', text)
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('TH3_VIP2:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local keko2 = redis:get('TH3_VIP2:'..bot_id..'hhh'..msg.sender_user_id_..'')
  if keko2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('TH3_VIP2:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
  redis:set('TH3_VIP2:'..bot_id..'help', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end

     if text:match("^الاوامر$") and is_mod(msg) then
  local help = redis:get('TH3_VIP2:'..bot_id..'help')
...
🎖┇م4 ~⪼ لعرض اوامر المطورين
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉]]
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('TH3_VIP2:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local keko2 = redis:get('tshake:'..bot_id..'h11'..msg.sender_user_id_..'')
  if keko2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('TH3_VIP2:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
  redis:set('TH3_VIP2:'..bot_id..'h1', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
     if text:match("^م1$") and is_mod(msg) then
  local h1 = redis:get('TH3_VIP2:'..bot_id..'h1')
     local text =  [[
📮┇ اوامر حمايه المجموعه بالمسح
  ...
  if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('TH3_VIP2:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local keko2 = redis:get('TH3_VIP2:'..bot_id..'h22'..msg.sender_user_id_..'')
   if keko2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('TH3_VIP2:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
   redis:set('TH3_VIP2:'..bot_id..'h2', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
     if text:match("^م2$") and is_mod(msg) then
   local h2 = redis:get('TH3_VIP2:'..bot_id..'h2')
...
📜┇رسائلي			
  ...
  if (text and text == 'تغير امر م3') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('TH3_VIP2:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local keko2 = redis:get('TH3_VIP2:'..bot_id..'h33'..msg.sender_user_id_..'')
   if keko2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('TH3_VIP2:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
   redis:set('TH3_VIP2:'..bot_id..'h3', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
  if text:match("^م3$") and is_mod(msg) then
  local h4 = redis:get('TH3_VIP2:'..bot_id..'h3')
...
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
    redis:set('TH3_VIP2:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
  return false end
    if text:match("^(.*)$") then
    local keko2 = redis:get('TH3_VIP2:'..bot_id..'h44'..msg.sender_user_id_..'')
    if keko2 == 'msg' then
    send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
    redis:set('TH3_VIP2:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
    redis:set('TH3_VIP2:'..bot_id..'h4', text)
    send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
  return
   false
 end
  ...
  local h4 = redis:get('TH3_VIP2:'..bot_id..'h4')
  ...
  🌐┇<strong>TH3_VIP2 TEAM</strong>
    
  🦁┇<a href="https://telegram.me/SCOURTEAM">قناه السورس</a>
  📜┇<a href="https://telegram.me/SCOURTEAM">قناه شروحات سورس</a>
  📁┇<a href="https://telegram.me/SCOURTEAM">قناه ملفات السورس</a>
    
  🔎┇<a href="https://t.me/SCOURTEAM/227">طريقه التنصيب</a>
...
local svuser = 'TH3_VIP2:'..bot_id..'user:'..result.id_
if username then
...
if database:get('TH3_VIP2:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('TH3_VIP2:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('TH3_VIP2:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:tshake"..msg.chat_id_..bot_id) then
local pin_id = database:get('TH3_VIP2:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local photo_keko = download_to_file('https://api.telegram.org/file/bot'..token..'/'..keko6.result.file_path, 'SCOURTEAM/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('TH3_VIP2:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:TH3_VIP2"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
...
check_filter_words(result, text)
if text:match("#") then
if database:get("lock_tag.note:TH3_VIP2"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("/")  then
if database:get("lock_sarha.note:TH3_VIP2"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:TH3_VIP2"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:TH3_VIP2"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
